...
//        v2RabbitAdmin.declareExchange(new DirectExchange("test.direct", false, false));
        v2RabbitAdmin.declareExchange(new TopicExchange("exchange.topic.example.new", false, false));
...
//        v2RabbitAdmin.declareQueue(new Queue("test.direct.queue", false));
        v2RabbitAdmin.declareQueue(new Queue("queue.example.topic.new", false));
...
//                "test.direct", "direct", new HashMap<>()));
        ...
        v2RabbitAdmin.declareBinding(
                BindingBuilder
                        .bind(new Queue("queue.example.topic.new", false))        //直接创建队列
                        .to(new TopicExchange("exchange.topic.example.new", false, false))    //直接创建交换机 建立关联关系
